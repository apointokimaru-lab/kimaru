...
6. **分析ダッシュボード**（`/analytics.html`・#343）
   - 経営判断と機能改善の優先順位づけに使う数字を1画面にまとめる。期間（7/30/90/365日）で全セクションが切り替わる。
   - **サマリー**: アカウント総数・有料転換率・MRR概算・期間内の登録数/予約数、登録と予約の日別推移。
   - **課金・転換**: 課金Pro/プレミアムの内訳、Cat Key の無償Pro（売上にならない有料会員）、登録→課金までの日数（中央値・四分位）、解約イベント、**登録月コホート別の転換率**（全体の率は母数の大きい古い月に薄まるため）。
   - **定着**: 稼働中アカウントのうち、予約ページ作成 → 受付時間設定 → カレンダー連携 → 予約が入った、までの到達率。急に落ちる段が改善対象。
   - **画面の利用状況**: 画面別PV/UU・画面×プラン・流入元・端末・獲得ファネル（LP→料金→登録画面→登録完了）・ゲストの予約ファネル。出どころは #342 の `page_events`。
   - 取得できなかった表は0ではなく「取得不可」として出す（0で見せると「使われていない」と読み違えるため）。
   - サイドメニューは3画面共通（`public/operator-nav.js` が単一ソース）。分析ダッシュボードだけアコーディオンで4つの子ビューを出し、別画面からでも目的のビューへ直接飛べる。
7. **将来**
