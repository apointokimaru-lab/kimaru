...
> **2026-09-05 の決定（#476）**: ① **Zoom は RTMS（Realtime Media Streams）で進める。参加者 Bot・Meeting SDK は使わない**（#370 / PR #473 で規約原文を確認）。② **音声取得＋文字起こしを先に、LLM 要約は後**（STT PoC は PR #474。LLM 準備 #379/#380 は後送）。③ **先に進めるのは Meet（ヘッドレスブラウザ Bot・#478）。Zoom RTMS（#475）は 2 番手として並行**——キマルは Google カレンダー連携を前提にしており、ほとんどのユーザーが Meet を使える状態から始まるため。反映先は `system-spec.md` v2.5 の 9章・FR-2・8章。
...
| 対象プラットフォーム | **Meet + Zoom 両対応**。**Meet 先行（M1・ヘッドレスブラウザ Bot・#478。Bot 用 Google アカウントを予定の参加者に招待して「参加をリクエスト」なしで入る仮説を検証。成り立たなければホストが admit）→ Zoom は RTMS（Bot は入室せず、ホストの承認のもとホストの Zoom から音声を受ける・#475）を 2 番手として並行**（2026-09-05 決定・#370/#476。`system-spec.md` 9章） |
| 先行順序 | **音声取得＋文字起こしを先に、LLM 要約は後**（2026-09-05 決定）。Meet Bot PoC は #478（`poc/meet-bot/`）、Zoom RTMS 受信 PoC は #475（`poc/rtms/`）、STT PoC は PR #474（`poc/stt/`・small int8 CPU で RTF 0.19〜0.23）で先行。LLM 準備（#379/#380）は M2 の後 |
...
| **35** | **Meet の招待入室の仮説**——Bot 用 Google アカウントを予定の参加者に招待すれば「参加をリクエスト」を経ずに入室できるか（成り立てばホストの毎回の許可操作が不要） | **#478（Meet Bot PoC）で招待済み／未招待の両方を実機確認** |
...
| **v2.5** | **Zoom を RTMS 化（参加者 Bot 廃止）・Meet 先行（招待入室の仮説 #478）・Zoom RTMS は 2 番手で並行・「音声取得＋文字起こし先行、LLM 後」の順序**を反映（2026-09-05・#476。T-304 の CPU 実測を追記。**Codex レビューは未実施**） |
