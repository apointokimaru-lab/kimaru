...
- **backfill（任意）**: [`sql/2026-08-04-availability-per-page-backfill.sql`](../../sql/2026-08-04-availability-per-page-backfill.sql)。
  共有行を各ページへ複製して「1ページ＝1セット」を明示的に作る。**不具合の修正には不要**（フォールバックで動く）で、**必ずデプロイ後に**実行すること（デプロイ前のコードは保存時に owner 単位で全削除するため消える）。
