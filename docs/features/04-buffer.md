...
- **バッファを設定したら「カレンダー予定名」は必須**（2026-08-14・#321）。空だと `createBufferEvent` が動かず、バッファ予定がカレンダーに出ないまま枠だけ塞がる＝「設定したのに何も起きない」状態になるため。画面側は `required`（バッファ0の側は欄ごと隠して `required` も外す）、サーバ側は `booking-page-save.js` が 400 を返す。既存データの遡及修正はしていないので、古いページを編集して保存するときに入力を求められる。
