...
| 2 | 料金・プラン（旧Pro版紹介） | `/plan.html` | 全員（未登録中心） | 不要 | 共通（guestナビ） |
...
| 14 | 分析ダッシュボード | `/analytics.html` | 運営者 | 運営セッション | 運営専用（最小） |
...
- **#14 分析ダッシュボード**: 利用状況・無料→有料の転換率・機能への定着を集計して表示（[features/22](./features/22-admin-console.md)・#343）。数字の出どころは既存テーブルと、画面計測 `page_events`（#342）。
...
| 種別 | 対象 | 内容 | 実装 |
|---|---|---|---|
| **共通** | A（認証前）/ B（認証後）/ E（法務） | `data-auth` により guest ナビ（料金/無料登録/ログイン）と app ナビ（ホーム/相手管理/予約設定/プロフィール/AIアシスト/設定）を出し分け | Edge Function `auth-gate.js` が `<!-- site-header -->` を注入。CSS `.guest-only`/`.app-only` で表示制御 |
| **運営専用** | C（運営） | ブランド＋「運営コンソール」ラベル＋言語のみ。ユーザー向けナビは非表示 | 各運営HTMLに直書き（共通ヘッダーは注入しない）。**左のサイドメニューだけは `public/operator-nav.js` が単一ソース**で、3画面とも同じ項目が並ぶ（分析ダッシュボードは子ビューをアコーディオンで展開・#343） |
