...
- **backfill（任意・⛔ 2026-08-04 に「実施しない」と判断。dev・本番とも未適用）**: [`sql/2026-08-04-availability-per-page-backfill.sql`](../../sql/2026-08-04-availability-per-page-backfill.sql)。
  共有行を各ページへ複製して「1ページ＝1セット」を明示的に作るもの。**不具合の修正には不要**（自前の行を持たないページは `booking_page_id = null` の共有行にフォールバックして従来どおり動く）ため見送った。
  将来あらためて揃えたくなったときのためにファイルは残してある（冪等・**必ずデプロイ後に**実行すること。デプロイ前のコードは保存時に owner 単位で全削除するため消える）。
- **注意**: 旧コードが上書きしてしまった受付時間は DB に元の値が残っていないため、コードでもSQLでも復元できない。
  デプロイ後、各予約ページを開いて意図した受付時間を入れ直して保存する必要がある。
