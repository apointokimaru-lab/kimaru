...
|---|---|
| 概要 | キマルとは／予約ページとは |
| 予約受付の準備 | Googleカレンダーの連携（2）／Zoomの連携（2）／予約ページの作成方法（4）／受付時間の設定方法／前後バッファの設定方法／事前アンケートの設定方法／アンケート回答の確認方法／プロフィールの設定（2） |
| 案内から面談当日まで | 予約ページの共有方法／ピンポイント日程調整（2）／予約確定後の流れ／キャンセル・日程変更への対応／予約の受付を一時的に止める |
...
| 一覧の見せ方 | **項目名だけのボタン**（要約も図も出さない） | 18項目あるので、1件あたりを短くして一覧性を優先する |
...
- 一覧に検索・絞り込みは無い（18項目・4章なのでスクロールで足りる）。これ以上増えるなら検索を足すかを決め直す。
