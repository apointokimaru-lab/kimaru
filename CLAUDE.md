...
### 新LP（`public/lp.html`・#364）
`docs/positioning-brief.md`（#362）を唯一の情報源にした登録獲得用のLP。**アプリのデザインシステム（`styles.css`）を読み込まず、`lp.css` / `lp.js` だけで完結する**（LPは明るい面・太ゴシック・角丸・自作SVGのイラストで作っており、アプリ側のトークンと混ぜると両方濁るため。朱 `#DE4A2E` だけ共通にして、登録後の画面と地続きに見せる）。**イラストは手描きのインラインSVGで、生成画像は使わない**（差し替えるときも同じ）。共通ヘッダーも i18n も使わない（日本語のみ・`data-i18n` を持たないので3言語の対称性の対象外）。守ること: **CTAは「無料で始める」の1本**（合否が「LP閲覧→会員登録」の転換率なので、出口を増やさない）、**ブリーフに無い機能・数値・導入社数を書かない**（実績値は存在しない）、**他社名を出さない**。**別ドメインへ移さない**——Edge が `usage.js` を差し込むのは自サイトのHTMLだけで、移すと分母（LPの閲覧数）と `owners.signup_source` が同時に切れる。`/lp.html` は `usage-summary.js` の獲得ファネルの「LP（トップ）閲覧」に合算しているので、パスを変えるならそこも直す。
