...
| 主要数値（8枚・4×2） | 登録ユーザー／有料ユーザー／課金転換率／MRR概算 ／ 初期設定が未完了／画面のアクセス（直近30日）／予約の累計／承認待ちCat Key | 並びは **人 → 使われ方 → 運営の宿題**。列数は固定（`auto-fit` だと幅しだいで1行7枚などに割れ、この並びの意味が消える） |
| 内訳・推移 | プランの構成（ドーナツ）／初期設定の進み具合（ドーナツ）／画面のアクセス（折れ線・直近30日） | ドーナツは正方形・折れ線は横長で必要な幅が違うので、3等分にはしない |
