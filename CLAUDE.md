...
- **使い方ガイド**は **`/guide.html`（機能一覧・文字だけ）＋ 項目ごとの説明Modal**で、実体は `public/guide.js`。共通ヘッダーの「使い方ガイド」(`href="/guide.html"`) は一覧へ送り、一覧の行を押すとその項目の説明が開く（`/guide.html#zoom-connect` の直リンクでも開く）。**`guide.js` を読むのは `guide.html` だけ**（Edge Function による全HTMLへの注入はやめた）。**説明の型は項目ごとに違う**——`ENTRIES` で `lead`（要約・一覧にも出す）/ `points`（概要の箇条書き）/ `steps`（手順）/ `fields`（設定項目の名前と説明）/ `note`（注記）を選ぶ。必要な i18n キーはこの指定から機械的に決まり、`scripts/test/unit.mjs` が3言語ぶん確認する。**項目名を一覧のHTMLに書かない**（`ENTRIES` と `GROUPS` が唯一の出どころ。2か所に持つと1つ足したとき必ず片方が古くなる）。骨格SVGの図は廃止（項目が「操作」に細かくなり、図が手順の言い換えにしかならなくなったため）。**文体は公的な手引きに準じた です・ます調**——条件は「〜する場合」、動作は「〜します／〜できます」。初期設定カードの説明文も同じ文体に揃える。
