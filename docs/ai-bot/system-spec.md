...
**対象**: 予約確定 → Bot 起動 → 音声取得 → バッチ文字起こし → AI要約 → 承認付き CRM 反映／利用時間の計測と枠管理／録音同意／音声・文字起こしの自動削除
...
| FR-6.2 | 課金対象は**入室から退出まで**。準備・待機室・アップロード・文字起こし・要約は含めない |
...
| `WAITING_ROOM_TIMEOUT` | 待機室15分 | 終了 | なし |
| `BOT_REJECTED` | 入室拒否 | 終了 | なし |
| `AUDIO_CAPTURE_FAILED` | 音声取得失敗 | 1回再試行→終了 | 入室後なら課金 |
| `NETWORK_DISCONNECTED` | 通信断 | 1回再入室（世代+1） | 課金 |
...
| 指標 | 目標 | 分母／除外 |
|---|---:|---|
| 入室成功率 | 95% | 分母＝起動した試行。除外＝プラットフォーム全体障害・ホスト拒否・無効URL |
...
| Bot タスク | 会議参加・音声取得 | **Fargate**（1会議1タスク） | 同左 |
...
課金対象は入室〜退出のみ。**残高は負数にならない**（枠を使い切った時点で Bot が退出するため）。課金月は JST 月初0時、会議は開始時刻の属する月に全量帰属。
...
| NFR-1.2 Bot 入室成功率 | 95% | 分母＝起動した試行。除外＝プラットフォーム全体障害・ホスト拒否・無効URL・同意なし。窓＝暦月・最低30件 |
...
| ID | 要件 |
|---|---|
| SEC-3.1 | Bot に渡すのは `meeting_id` / `meeting_url` / `bot_name` / 時刻制限 / **状態更新トークン**のみ |
...
| SEC-3.4 | 会議URLを scheme・許可ホスト・リダイレクト先で検証 |
| SEC-3.5 | **アウトバウンドを許可先に限定**（会議プラットフォーム・S3・状態更新API）。**169.254.169.254 を遮断** |
| SEC-3.6 | インバウンド禁止 |
| SEC-3.7 | 本番で ECS Exec 無効 |
| SEC-3.8 | 会議終了後にコンテナ破棄（再利用しない） |
| SEC-3.9 | Chromium を定期更新 |
...
CI で依存脆弱性検査／ECR イメージスキャン／**Chromium 定期更新**／Critical 7日・High 30日以内。
...
1 Bot が入室できない／2 **Scheduler の配送が止まった**／3 **SQS が大量再送されている**／4 **古い Bot が停止していない（fencing 発動後）**／5 STT キュー滞留／6 **LLM 障害・レスポンス不正**／7 **LLM 費用暴走**／8 音声削除失敗／9 残高の手動調整／10 シークレット露出／11 Supabase 障害時の縮退運転／12 デプロイのロールバック／13 復元試験／14 個人情報削除依頼／15 **誤録音・同意事故（全トークン失効を含む）**
...
> **前提**: ¥155/USD。Fargate 4vCPU/8GB = ¥38.2/時間。GPU は **Fargate では使えない**（GPU リソース要求は EC2 起動タイプ専用）ため、g4dn.xlarge（T4）¥110/時間・g5.xlarge（A10G）¥156/時間のオンデマンド概算を用いる。**RTF と起動時間は公開情報からの想定値であり、T-304 で実測して置き換える。**
...
| M1 | Bot PoC | **並行性試験11シナリオが全通過**し、入室・音声取得ができる |
...
| T-208 | Bot コンテナ（Playwright・入室・URL検証） | T-001 | 入室成功率を測定できる |
...
| T-302 | faster-whisper CPU ワーカー（モデル同梱・アイドル終了・複数ジョブ処理） | T-301 | 起動費を償却できる |
| T-303 | 完了判定（`completed` / `_with_gaps` / `incomplete`）と音声削除 | T-210,T-302 | **不完全な音声で削除が起きない** |
| T-304 | CPU/GPU ベンチマーク | T-302 | **RTF・固定起動費・α・原価・日本語品質・固有名詞誤り数**を記録 |
...
- [ ] 入室成功率・音声欠損率・終了検知成功率・途中切断率（分母と除外を定義して）
- [ ] **α の実測**（事前起動・待機室・会議・終了処理・失敗試行の内訳）
- [ ] **会議時間の分布**（短時間会議のオーバーヘッド比率）
- [ ] faster-whisper CPU/GPU の RTF・**固定起動費**・実費・日本語品質・固有名詞誤り数
...
> 🔎 **T-001 の一次調査が完了した**（[`platform-research.md`](./platform-research.md)）。結果、**先行順序を Zoom に変えることを提案**している。理由:
> - Zoom は **Meeting SDK for Linux による headless bot が公式ルート**で、Notta・Fireflies・Otter が Marketplace 掲載済み
> - Meet は **Media API が Developer Preview（全参加者の登録が必要で使えない）**、**Meet SDK の bot-free 方式は Google Workspace 管理者権限が必要**でキマルの顧客層に届かない → **ヘッドレスブラウザしか選択肢がなく、規約グレー・UI変更で壊れるリスクを先に踏む**
> - Zoom は **Legal UI Notices（Active Apps Notifier 等）が必須**で、同意の仕組みが標準装備されている
>
> **下表の「Meet 先行」は、この提案の採否が決まるまでの暫定。**

| 段階 | 対象 | 理由 |
|---|---|---|
| PoC・M1 | **Google Meet 先行** | Zoom は Marketplace 審査中で、Bot 用スコープの追加が**再審査**を誘発する。審査完了を待つ間に Meet で技術検証を進める |
| M2 以降 | **Zoom を追加** | 審査完了後にスコープを追加し、同じインターフェースで実装する |

**`MeetingPlatformAdapter` を最初から切る。** 入室・音声取得・参加者数取得・終了検知・退出を抽象化し、Meet 実装を先に作って Zoom 実装を後から差し込む。

```ts
interface MeetingPlatformAdapter {
  join(url: string, botName: string): Promise<void>;
  waitForAdmission(timeoutMs: number): Promise<boolean>;
  startAudioCapture(sink: SegmentSink): Promise<void>;
  getParticipantCount(): Promise<number>;   // Bot 自身を除いた人数
...
|---|---|---|
| 実装時期 | **M1（先行）** | **M2 以降** |
| 入室方式 | ヘッドレスブラウザ（Playwright + Chromium） | ヘッドレスブラウザ or Meeting SDK（T-001 で判断） |
| 音声取得 | 要検証 | 要検証 |
| 外部主催の会議 | 要検証 | 要検証 |
| 待機室 | 要検証 | 要検証 |
| 審査 | Google 側の条件 | **審査中**。Bot スコープ追加で再審査 |
| UI 変更への耐性 | ブラウザ依存（日次 E2E で監視） | 実装方式による |
| 規約 | T-001 で確認 | T-001 で確認 |

> ⚠️ **前提**: 両プラットフォームとも**規約上 Bot 参加が許容されること**が条件（T-001）。片方が不可と判明した時点で対応範囲を見直す。
...
| Meet/Zoom の UI 変更で入室不能 | Bot 全停止 | 日次の入室 E2E |
...
| Zoom 審査への影響 | 既存機能停止 | Bot スコープ追加は審査完了後 |
...
| # | 論点 | 決め方 | 期限 |
|---:|---|---|---|
| ~~1~~ | ~~Bot 対象は Meet か Zoom か~~ | ✅ **決定済み（2026-08-06）**: **両対応**。Meet 先行（M1）→ Zoom 追加（M2 以降）。9章 | — |
| 2 | 文字起こしは自前CPU / GPU | **7.2.2.1 の比較表**を判断材料に、T-304 で品質と RTF を実測して確定 | M2 |
