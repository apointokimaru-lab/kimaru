...
**対象**: 予約確定 → 取得タスク起動 → 音声取得（**Zoom: RTMS 受信／Meet: Bot 入室**・9章） → バッチ文字起こし → AI要約 → 承認付き CRM 反映／利用時間の計測と枠管理／録音同意／音声・文字起こしの自動削除
...
> | FR-2.2 待機室15分 | 待機室でホストの admit を待つ | **ホストがアプリのアクセスを承認**するまで待つ（承認要求は Zoom クライアントがホストに出す）。15分で `WAITING_ROOM_TIMEOUT` 相当 |
...
| 指標 | 目標 | 分母／除外 |
|---|---:|---|
| 入室成功率（**Zoom はストリーム開始成功率**＝ホスト承認後に音声が届いた率） | 95% | 分母＝起動した試行。除外＝プラットフォーム全体障害・ホスト拒否・無効URL・**ホスト側で RTMS が無効** |
...
| M1 | Bot PoC（音声取得） | **並行性試験11シナリオが全通過**し、音声取得ができる（**Zoom: RTMS 受信／Meet: Bot 入室**） |
...
**先行順序（2026-09-05 決定）**: **M1（音声取得）→ M2（文字起こし）を先に通し、M3（要約・LLM）は後。** LLM のアカウント・キー・上限設定（#379 / #380）は M2 の完了判定が出てから着手する（先に契約すると使わない月額・上限設定が発生し、要約の仕様も文字起こしの実物を見てから決めた方がよいため）。M1 のうち Zoom の取得は #475（RTMS 受信 PoC・`poc/rtms/`）、M2 のうち CPU の文字起こし・完了判定・ベンチマークは #393（PR #474・`poc/stt/`）で、**AWS 基盤（M0・#382）の完了を待たずローカルで着手済み**。「動くものを先に見る」順にした。
...
| T-208 | 取得タスク。**Zoom: RTMS 受信**（webhook `meeting.rtms_started`／`rtms_stopped` の署名検証 → シグナリング WebSocket → メディア WebSocket → 音声受信。**#475 の PoC に置き換え・2026-09 着手**）／**Meet: Bot コンテナ**（Playwright・入室・URL検証。ホストが admit する運用前提） | T-001 | Zoom: ストリーム開始成功率、Meet: 入室成功率を測定できる |
...
- [ ] 入室成功率（Zoom: ストリーム開始成功率）・音声欠損率・終了検知成功率・途中切断率（分母と除外を定義して）
...
> ✅ **2026-09-05 決定（#370 / PR #473・#476）: Zoom 先行、方式は RTMS（Realtime Media Streams）。参加者 Bot は使わない。**
...
> - **Meet**: ヘッドレスブラウザ Bot しか選択肢がないのは変わらない。Google の規約に Bot 入室を禁じる明文はないが、**Google は製品側で「third-party bots, like note takers」の「参加をリクエスト」を自動拒否できる設定を案内**しており、ホストが「Anyone can ask to join」を OFF にしていると入室できない。採るなら **ホスト（＝キマルのユーザー）が Bot を自分で admit する運用**を前提にし、**Bot 検知を回避する実装（人間を装う UA・挙動）はしない**（ToS の "bypassing our systems or protective measures" に抵触する）。Media API は Developer Preview のまま
> - 両方とも **TypeScript で書ける**（Zoom は WebSocket、Meet は Playwright）。C++ の Meeting SDK 実装は不要になった
> - v2.4 の「Meet 先行（暫定）」は本節で置き換える。「Zoom は Bot スコープの追加で再審査を誘発するので Meet 先行」という理由は、**RTMS のスコープは PoC 用の別アプリで試し（#475）、本番アプリへは #388 を RTMS のスコープで出し直す**ことで解消する

| 段階 | 対象 | 理由 |
|---|---|---|
| PoC・M1 | **Zoom 先行（RTMS）** | 規約・技術の両面で固い（公式が Bot の代替として案内する経路）。Bot の入室・待機室・UI 変更耐性の問題が消える。PoC は開発用の別 Zoom アプリで進め、審査済みの本番アプリに触らない（#475） |
| M2 以降 | **Meet を追加（ヘッドレスブラウザ Bot）** | ホストが admit する運用と日次 E2E を前提に、「規約違反ではないが排除されている経路を踏む」リスクを承知で採る。Media API の GA を待つ選択肢も残す |
...
| 待機室 | 無し。代わりに**ホストのアプリ承認**（15分で `WAITING_ROOM_TIMEOUT` 相当） | 待機室 15分 |
...
| ホスト側の条件 | 「Share realtime meeting content with apps」ON・アカウント verified（二次情報）。無料プランで可（二次情報） | ホストが admit できること |
...
> ⚠️ **前提**: Zoom は規約上の許容が確定した。Meet は「規約違反ではないが Google が製品側で排除している経路」であり、**ホストが admit する運用で成立するか**を M2 の PoC で確認する。成立しなければ Meet は Media API の GA まで対応範囲から外す。
...
| Meet の UI 変更・Bot 検知で入室不能（Zoom は RTMS なので該当しない） | Meet の Bot 全停止 | 日次の入室 E2E。ホストが admit する運用。Bot 検知の回避はしない |
...
| # | 論点 | 決め方 | 期限 |
|---:|---|---|---|
| ~~1~~ | ~~Bot 対象は Meet か Zoom か~~ | ✅ **決定済み（2026-08-06）**: **両対応**。→ **2026-09-05 更新: Zoom 先行（RTMS・参加者 Bot なし）、Meet はヘッドレスブラウザ Bot を M2 以降**（#370 / PR #473 / #476）。9章 | — |
