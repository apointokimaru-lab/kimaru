...
// ---------- 1b) 使い方ガイドの導線（#353・#423）----------
// ガイド本体（項目・ページ・文言キーの対応）は Next に移したので features/guide/entries.test.ts が見る。
// ここに残すのは旧ページ側の導線だけ——共通ヘッダー（Edge が注入）とダッシュボードの初期設定カードの2か所が、
// どちらも新 URL /guide（一覧）へ送れていること。
section("user guide links (#353・#423)");
const authGate = fs.readFileSync(path.join(repo, "netlify/edge-functions/auth-gate.js"), "utf8");
ok("header guide link goes to the list page", authGate.includes('href="/guide" data-i18n="nav.guide"'));
const dashHtml = fs.readFileSync(path.join(repo, "public/dashboard.html"), "utf8");
ok("setup card guide button goes to the list page", /href="\/guide"[^>]*data-i18n="setup\.guide"/.test(dashHtml));
...
section("usage tracking: path normalization (#342)");
{
  ...
  ok("Next に移した /privacy は旧の集計キー /privacy.html に揃う（#421）", n("/privacy") === "/privacy.html");
  ok("Next に移した /tokushoho は旧の集計キー /tokushoho.html に揃う（#422）", n("/tokushoho") === "/tokushoho.html");
