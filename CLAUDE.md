...
- **使い方ガイド**は **`/guide.html`（機能一覧＝項目名だけのボタン）＋ 項目ごとの説明Modal**で、実体は `public/guide.js`。共通ヘッダーの「使い方ガイド」(`href="/guide.html"`) は一覧へ送り、ボタンを押すと**その項目だけ**の説明が開く（`/guide.html#zoom-connect` の直リンクでも開く）。**Modalに送り（前へ/次へ）は無い**——どの項目を押しても同じ器が開くと、どこまでがその説明か分からなくなるため。**1項目は iPhone 12（390×664）でスクロールせずに収まる量に抑える**（`scripts/test/e2e.mjs` が実測で固定）。**収まらなくなったら文を削るのではなく項目を分ける**（手順を削ると「読んでも設定できない」に戻る）。**説明の型は項目ごとに違う**——`ENTRIES` で `lead`（要約）/ `points`（概要の箇条書き）/ `steps`（手順）/ `fields`（設定項目の名前と説明）/ `note`（注記）を選ぶ。必要な i18n キーはこの指定から機械的に決まり、`scripts/test/unit.mjs` が3言語ぶん確認する。**項目名を一覧のHTMLに書かない**（`ENTRIES` と `GROUPS` が唯一の出どころ）。**文体は公的な手引きに準じた です・ます調**——条件は「〜する場合」、動作は「〜します／〜できます」。初期設定カードの説明文も同じ文体に揃える。
